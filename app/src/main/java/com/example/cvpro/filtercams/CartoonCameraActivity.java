 ...
 *   카툰 필터 효과 CAM 액티비티
 */
public class CartoonCameraActivity extends AppCompatActivity
    ...
    public native void ConvertRGBtoCartoonTwo(long matAddrInput, long matAddrResult);
    public native void ConvertRGBtoCartoonThree(long matAddrInput, long matAddrResult);
    public native void ConvertRGBtoCartoonFour(long matAddrInput, long matAddrResult, long matAddrCapture);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mOpenCvCameraView = (CameraBridgeViewBase)findViewById(R.id.activity_cartoon_surface_view);
    ...
    public void onResume()
    {
        ...
        if (!OpenCVLoader.initDebug()) {
            Log.d(TAG, "onResume :: opencv 라이브러리 찾을 수 없음");
            OpenCVLoader.initAsync(OpenCVLoader.OPENCV_VERSION_3_2_0, this, mLoaderCallback);
        } else {
            Log.d(TAG, "onResume :: opencv 라이브러리 성공적으로 로드, 사용 가능");
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        try {
            // 1. 흑백 필터 적용 -> 2. 블러 적용(노이즈 제거용) -> 3. 윤곽선 추출 -> 4. 색 반전 (4단계에서 캡처용 MAT에 저장)
            ConvertRGBtoCartoon(matInput.getNativeObjAddr(), matResult.getNativeObjAddr());
            ConvertRGBtoCartoonTwo(matResult.getNativeObjAddr(), matResult.getNativeObjAddr());
            ConvertRGBtoCartoonThree(matResult.getNativeObjAddr(), matResult.getNativeObjAddr());
            ConvertRGBtoCartoonFour(matResult.getNativeObjAddr(), matResult.getNativeObjAddr(), captureMat.getNativeObjAddr());
        } catch (Exception e){
            e.printStackTrace();
        }
        return matResult;
    ...
    private void showDialogForPermission(String msg) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder( CartoonCameraActivity.this);
        builder.setTitle("알림");
